...
<body class="hold-transition sidebar-mini">
                  ...
                  <table id="example1" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      <?php foreach ($fournisseurs as $fournisseur) : ?>
                        <tr>
                          <td style="padding-top:17px;"><?php echo $fournisseur['nomFournisseur'] ?></td>
                          <td style="padding-top:17px;"><?php echo $fournisseur['prenomFournisseur'] ?></td>
                          <td style="padding-top:17px;">
                            <a href="tel:<?php $tel = $fournisseur['telFournisseur'];
                                          echo $tel ?>"><?php echo $fournisseur['telFournisseur'] ?>
                            </a>
                          </td>
                          <td style="padding-top:17px;"><?php echo $fournisseur['adresseFournisseur'] ?></td>
                          <td style="padding-top:17px;">
                            <?php
                            if ($fournisseur['emailFournisseur'] == NULL) {
                              echo "Non défini";
                            } else {
                              $email = $fournisseur['emailFournisseur'];
                              echo "<a href='mailto:$email'>$email</a>";
                            }
                            ?>
                          </td>
                          <td style="padding-top:17px;"><?php echo $fournisseur['deliveryDelay'] ?></td>
  
                          
                          <td>
                            <button type="button" id="edit" data-type="edit" data-id="<?php echo $fournisseur['idFournisseur'] ?>" data-nom="<?php echo $fournisseur['nomFournisseur'] ?>" data-prenom="<?php echo $fournisseur['prenomFournisseur'] ?>" data-tel="<?php echo $fournisseur['telFournisseur'] ?>" data-adresse="<?php echo $fournisseur['adresseFournisseur'] ?>" data-email="<?php echo $fournisseur['emailFournisseur'] ?>" data-struct="<?php echo $fournisseur['nomStructFournisseur'] ?>" data-social="<?php echo $fournisseur['raisonSocialeFournisseur'] ?>" data-deliverydelay = <?php echo $fournisseur['deliveryDelay'] ?> data-toggle="modal" data-target="#modal-lg" class="btn btn-success btn-sm"><i class="fa fa-edit"></i>
                            </button>
                            <button type="button" data-id="<?php echo $fournisseur['idFournisseur'] ?>" data-toggle="modal" data-target="#confirm-modal" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></button>
                          </td>
                        </tr>
                      <?php endforeach ?>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">Ajouter un fournisseur
          ...
          <form id="forms" method="POST">
            <div class="container modal-body">
              <div class="form-row">
                ...
                <div class="form-group col-md-4">
                  <label for="prenom">Prénom(s) du fournisseur</label>
              ...
              <div class="form-row">
                <div class="form-group col-md-4">
                  <label for="social">Raison sociale <span class="text-muted">(Optionnel)</span></label>
                  <input type="text" class="form-control" name="social" id="social" placeholder="Nature de la structure">
                </div>
                <div class="form-group col-md-4">
                  <label for="deliveryDelay">Délai de livraison</label>
                  <input type="text" class="form-control" name="deliveryDelay" id="deliveryDelay" placeholder="En jours">
                </div>
              </div>
  ...
  <script>
    ...
    $('#modal-lg').on('show.bs.modal', function(event) {
      ...
      if (button.data('type') === 'ajout') {
        ...
        var deliveryDelay = button.data('deliverydelay');
        $('#id').val(id);
        $('#nom').val(nom);
        $('#prenom').val(prenom);
        $('#tel').val(tel);
        ...
        $('#social').val(social);
        $('#struct').val(struct);
        $('#deliveryDelay').val(deliveryDelay);
        $('#update-btn').show();
        $('#save-btn').hide();
      }
  ...
  <?php if (isset($_POST['submit']) || isset($_POST['update'])) : ?>
    <?php if (isset($_POST['submit'])) : ?>
      ...
      $struct = $_POST['struct'];
      $social = $_POST['social'];
      $delivery = $_POST['deliveryDelay'];
      $fournisseur = new Fournisseur($nom, $prenom, $email, $tel, $adresse, $struct, $social, $delivery);
      Fournisseur::create($fournisseur);

      echo "<meta http-equiv='refresh' content='1'>";
      ?>
      <script>
        var text = 'Fournisseur ajouté avec succès !';
  ...
  <?php if (isset($_POST['delete'])) : ?>
    ...
    ?>
    <script>
      var text = 'Fournisseur supprimé avec succès !';
    ...
    $(function() {
      ...
      $('#forms').validate({
        ...
        messages: {
          ...
          prenom: {
            required: "Le prénom du fournisseur est requis !",
          },
          tel: {
            required: "Le contact du fournisseur est requis !",
          },
          deliveryDelay: {
            required: "Le délai de livraison du fournisseur est requis !",
          },
          adresse: {
            required: "L'adresse du fournisseur est requis !",
          },
          email: {
            required: "",
          },
